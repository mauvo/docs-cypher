...
| xref:expressions/predicates/comparison-operators.adoc#is-not-null-operator[`IS NOT NULL`], xref:values-and-types/working-with-null.adoc#is-null-is-not-null[Working with null]
| GQL allows testing whether a value (typically a property) exists and is not null.
...
This can be used in xref:clauses/where.adoc[`WHERE`] clauses, xref:clauses/filter.adoc[`FILTER`] clauses, and xref:constraints/managing-constraints.adoc#create-property-existence-constraints[property existence constraints] (`REQUIRE ... IS NOT NULL`).
For index use with existence checks, see xref:indexes/search-performance-indexes/using-indexes.adoc#property-existence-check[Using indexes -> Property existence checks].
